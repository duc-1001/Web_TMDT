...
import { SignupForm, signupSchema } from "@/schemas/singup.schema"
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/10 via-background to-primary/5 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Đăng ký để nhận ưu đãi và trải nghiệm mua sắm tuyệt vời
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {submissionMessage.text && (
              <div className={`p-3 rounded-lg text-sm ${submissionMessage.type === 'error' ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'
                }`}>
                {submissionMessage.text}
              </div>
            )}
