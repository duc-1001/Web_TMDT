...
const EditBrand = (props: EditBrandProps) => {
    const { setEditDialogOpen, brand } = props
    const {
        register,
        handleSubmit,
    ...
    const editBrandMutation = useMutation({
        mutationFn: ({ id, data }: { id: string, data: BrandForm }) => editBrand(id, data),
        onSuccess: (data) => {
    ...
    const onSubmit = async (data: BrandForm) => {
        try {
            await editBrandMutation.mutateAsync({ id: brand._id, data });
        } catch (error) {
        ...
        <DialogContent className="max-w-md">
            <form onSubmit={handleSubmit(onSubmit)}>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Chỉnh sửa thông tin thương hiệu cho cửa hàng của bạn
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="w-16 h-16 rounded-lg relative border bg-muted flex items-center justify-center overflow-hidden">
                                {watch("logo") ? (
                                    <>
                                        <img
                                            src={watch("logo") instanceof File ? URL.createObjectURL(watch("logo") as File) : (watch("logo") as string)}
                                            alt="Logo preview"
                ...
                <DialogFooter>
                    <Button disabled={isSubmitting} type='button' variant="outline" onClick={() => setEditDialogOpen(false)} className="bg-transparent">
                        Hủy
                    </Button>
