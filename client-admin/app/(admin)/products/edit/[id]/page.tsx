...
import { ProductFormEdit, productSchemaEdit } from "@/schemas/product.schema"
import { zodResolver } from "@hookform/resolvers/zod"
import { createProduct, editProduct, getProductByIdAdmin } from "@/services/product.service"
...
export default function EditProductPage({ params }: EditProductPageProps) {
    ...
    const { data: product } = useQuery({
        queryKey: ["detail-product-for-edit", id],
    ...
    useEffect(() => {
        if (product) {
            ...
            setValue("category", product.category ? { _id: product.category._id, name: product.category.name } : { _id: "", name: "" })
            setValue("brand", product.brand ? { _id: product.brand._id, name: product.brand.name } : { _id: "", name: "" })
            setValue("sku", product.sku || "")
            setValue("weight", product.weight)
            setValue("unit", product.unit)
            setValue("ingredient", product.ingredient || "")
            setValue("allergens", product.allergens || [])
            setValue("storageInstruction", product.storageInstruction || "")
    ...
    const handleSubmit = async (data: ProductFormEdit) => {
        const payload = {
            ...data,
